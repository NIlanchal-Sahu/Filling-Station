...
function mapDip(id: string, data: DocumentData, fuelName = ''): FuelTankDipReading {
  ...
  const dipCm =
    dipCmRaw != null ? Number(dipCmRaw) : (dipCmFromLiters(dipLiters, fuelName) ?? 0);
  const pumpDayIso =
...
/** Preview stock liters from a dip-stick reading before saving. */
...
export async function recordFuelTankDip(input: {
  ...
  const dipLiters = litersFromDipCm(input.dipCm, fuel.name);
